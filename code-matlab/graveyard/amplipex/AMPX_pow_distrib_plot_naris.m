function AMPX_pow_distrib_plot_naris(power, trial_data, data, varargin)
...
if min_freq<65 && min_freq> 20; type = 'low_gamma'; clim = [0 900]; type_title = 'low gamma'; raw_plot_boost = 200;line_end  =.5; % use a rescale function to see the data in a different way.  use this as a flag
elseif min_freq>66 && max_freq<100; type = 'high_gamma'; type_title = 'high gamma'; clim = [10 300]; raw_plot_boost = 200; line_end  =.5;
...
for itrial = 1:length(trial_data.trial)
    %%
    figure(1000);
    maximize
    subplot(1,6,1:2)
    ...
    hline(0, 'k')
    rectangle('position',[.5*line_end-50, 10, .2*line_end, 25], 'facecolor', [.6 .6 .6], 'edgecolor', [.6 .6 .6])
    if strcmp(type, 'high_gamma')
        ...
        text(.5*line_end-50, -35, 'Time (ms)','fontsize',ft_size-4, 'fontweight','bold', 'fontname', 'helvetica')
        text(.035, 25, 'Ventrolateral', 'fontsize',ft_size, 'fontweight','demi', 'fontname', 'helvetica');
        ...
        text(.035, (plot_loop-0.3)*raw_plot_boost, 'Dorsomedial', 'fontsize',ft_size, 'fontweight','demi', 'fontname', 'helvetica');
    ...
    % imagesc walkaround
    h = nan_imagesc_ec(power.power_distrib{1,itrial});
    %
    plt_ft_size = 42;
    text(1-.15,1,markers{4},'fontsize',plt_ft_size, 'fontweight','bold', 'color', 'w', 'BackgroundColor', 'none', 'Margin', 1.5);
    text(1-.15,8,markers{3},'fontsize',plt_ft_size, 'fontweight','bold','color', 'w', 'BackgroundColor', 'none', 'Margin', 1.5);
    ...
    tx = text(0.46,0.95,[data.hdr.Filename(5:end-4) ' ' type_title ' trial# ' num2str(itrial)]);
    ...
    target_dir = ['D:\DATA\' data.hdr.Filename(5:8) '\' data.hdr.Filename(5:19) '\' date '\' session_type '\' type '_pow_figures\' data.hdr.Filename(5:end-4) '_' num2str(itrial) '_spec.png'];
    export_fig(target_dir, '-native')
    if strcmp(save_fig,'on'); saveas(gcf,['D:\DATA\' data.hdr.Filename(5:8) '\' data.hdr.Filename(5:19) '\' date '\' session_type '\' type '_pow_figures\' data.hdr.Filename(5:end-4) '_' num2str(itrial) '_spec.fig']);end
    naris_dir =     ['G:\Naris\paper_figs\' data.hdr.Filename(5:end-4) '_' num2str(itrial) '_spec.png'];
