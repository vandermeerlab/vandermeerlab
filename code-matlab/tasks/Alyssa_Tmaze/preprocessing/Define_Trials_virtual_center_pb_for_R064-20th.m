...
title('Define center photobeam. Just make a small rectangle where it should be.'); % track minus undesirable areas, like very start, plus rewards...on second thought, include start box b/c true beginning of trial
[xCPB,yCPB] = ginput;
inCPB = inpolygon(xtask,ytask,xCPB,yCPB);
plot(xtask(inCPB),ytask(inCPB),'y');

clear xStart yStart xLstop yLstop xRstop yRstop xL yL xR yR xCPB yCPB
...
pbC_crossed = diff(inCPB)==1;
mafak = tpos(task_tidx); % because my variable naming in this script just couldn't get any worse. this is the segment of timestamps for the task
ts_pbC = mafak(pbC_crossed); % ts where virtual center photobeam was crossed
idx_pbC = cat(2,find(diff(ts_pbC)>5),length(ts_pbC));
