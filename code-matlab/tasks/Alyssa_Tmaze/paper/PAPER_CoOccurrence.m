...
% Tmaze project
% This script loads the inputData generated by the script PAPER_Collect_inputData
...
cfg.whichEvents = 'postrecord'; % 'all', 'prerecord', 'task', 'postrecord', 'allITI', 'equalBehaviorITI'
...
cfg.useMask = 0; % If 1, exclude pairs from the same tt, if 0, don't
...
for iRat = 1:length(cfg.rats)
    ...
    for iSession = 1:length(inputData.(cfg.rats{iRat}))
        ...
        for iArm = 1:length(arms)
            ...
            if ~isempty(sessionCC.(arms{iArm}).S.t) 
