...
cfg_def.NFFT = 1024;
cfg_def.ref_chan = ExpKeys.Ref_chan;
cfg_def.ref_loc = 'dm'; % which corner of the probe to use as a reference (can be 'vl' (ventrolateral), 'vm' (ventromedial), 'dl' (dorsolateral), 'dm' (dorsal medial)
cfg_def.output = 'rad'; % output in radians ('rad') or degrees ('deg')
cfg = ProcessConfig(cfg_def, cfg_in);
%% determine the reference electrode

[~,ref_loc] = Naris_BestChan_remap(ExpKeys);
...
for itrial = 1:length(data_ft.trial)
    for ii = 64:-1:1
        [Cxy,F] = cpsd(data_ft.trial{itrial}(ref,:),data_ft.trial{itrial}(ii,:),hamming(cfg.spec_window/2),cfg.spec_window/4,cfg.NFFT,data_ft.hdr.Fs);
