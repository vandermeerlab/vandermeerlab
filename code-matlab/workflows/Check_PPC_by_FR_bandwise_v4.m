...
%                 pct_ppc = sum(shuf_ppc < all_ppc, 1)/size(shuf_ppc,1); % Calculating p_val of PPC for each frequency
                % Test for significance in each of the frequency bands using mean ppc values 
                ...
                for iF = 1:length(f_list)
                    ...
                    mean_ppc = mean(all_ppc(f_idx));
                    this_shuf = mean(shuf_ppc(:,f_idx),2);
                    this_sig(iF) = mean_ppc >= prctile(this_shuf, 100*p_thresh);
                end
