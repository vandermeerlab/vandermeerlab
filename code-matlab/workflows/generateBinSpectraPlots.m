...
clear;
close all;
% load('/Users/manishm/Work/vanDerMeerLab/RandomVStrDataAnalysis/results_v2/R117-2007-06-01_sts.mat');
% c_list = {'k','b','g','r'};
% % for near-trials;
...
cd('/Users/manishm/Work/vanDerMeerLab/RandomVStrDataAnalysis/results_v2');
...
for idx = 1:length(rats)
    ...
    for jdx = 1:length(ofiles)
        ...
        for iC  = 1:length(od.S1.cell_type)
            ...
            subplot(2,1,1);
            title('STA spectrum for near trials','FontSize', 18);
            subplot(2,1,2);
        ...
        for iC  = 1:length(od.S2.cell_type)
            ...
            for iP = 1:4
                subplot(2,1,1);
                plot(od.S2.freqs,10*log(od.S2.spec_res(iC).sta_spec_ptile(iP,:)), c_list{iP});
                hold on;
                plot(od.S2.freqs,10*log(od.S2.spec_res(iC).w_sta_spec_ptile(iP,:)),cat(2,':',c_list{iP}));
                hold on;
                plot(od.S2.freqs,10*log(od.S2.spec_res(iC).uw_sta_spec_ptile(iP,:)),cat(2,'--',c_list{iP}));
                hold on;
                subplot(2,1,2);
                plot(od.S2.freqs,10*log(od.S2.spec_res(iC).sts_ptile(iP,:)),c_list{iP});
                hold on;
