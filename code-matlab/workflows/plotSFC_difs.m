...
% lfq-hfq hZ) for HFR and LFR trials
lfq = 40;
hfq = 95;
...
for idx = 1:length(rats)
    ...
    for jdx = 1:length(ofiles)
        ...
        for iC  = 1:length(od.S1.msn_res)
            ...
            msn_near_sfc_difs = [msn_near_sfc_difs, od.S1.freqs(fq1+id_hf-1) - od.S1.freqs(fq1+id_lf)];
        end
        ...
        for iC  = 1:length(od.S1.fsi_res)
            ...
            fsi_near_sfc_difs = [fsi_near_sfc_difs, od.S1.freqs(fq1+id_hf-1) - od.S1.freqs(fq1+id_lf-1)];
            fsi_near_usfc_difs = [fsi_near_usfc_difs, od.S1.freqs(fq1+uid_hf-1) - od.S1.freqs(fq1+uid_lf-1)];
        ...
        for iC  = 1:length(od.S2.msn_res)
            ...
            [~,id_lf] = max(sfc_lf(fq1:fq2));
            [~,id_hf] = max(sfc_hf(fq1:fq2));
            msn_away_sfc_difs = [msn_away_sfc_difs, od.S2.freqs(fq1+id_hf-1) - od.S2.freqs(fq1+id_lf-1)];
        end
        ...
        for iC  = 1:length(od.S2.fsi_res)
            ...
            [~,uid_lf] = max(usfc_lf(fq1:fq2));
            [~,uid_hf] = max(usfc_hf(fq1:fq2));
            fsi_away_sfc_difs = [fsi_away_sfc_difs, od.S2.freqs(fq1+id_hf-1) - od.S2.freqs(fq1+id_lf-1)];
            fsi_away_usfc_difs = [fsi_away_usfc_difs, od.S2.freqs(fq1+uid_hf-1) - od.S2.freqs(fq1+uid_lf-1)];
