            ...
            if isfield(od.fsi_res.near_spec{iC}, 'flag_no_control_split') && ~od.fsi_res.near_spec{iC}.flag_no_control_split
                ...
                if any(this_sig) % Go ahead only if any band passes the significance critetion
                    ...
                    for iF = 1:length(f_list)
                        ...
                        if ~isnan(fsi_summary.hfr_ppc_diff(sidx)) % If the HFR-LFR frequency diff was significant
                                if ismember(fsi_summary.hfr_ppc_diff(sidx), od.fsi_res.near_spec{iC}.freqs(f_idx))
                                   scatter(ax{iF}, hi_mfr, mean(hfr_ppc(f_idx)), msz, c_list{iF})
                                end
                        end
                        if ~isnan(fsi_summary.lfr_ppc_diff(sidx)) % If the HFR-LFR frequency diff was significant
                                if ismember(fsi_summary.lfr_ppc_diff(sidx), od.fsi_res.near_spec{iC}.freqs(f_idx))
            ...
            if isfield(od.msn_res.near_spec{iC}, 'flag_no_control_split') && ~od.msn_res.near_spec{iC}.flag_no_control_split
                ...
                if any(this_sig) % Go ahead only if any band passes the significance critetion
                    ...
                    for iF = 1:length(f_list)
                        ...
                        plot(ax{iF}, [low_mfr, hi_mfr], [mean(lfr_ppc(f_idx)), mean(hfr_ppc(f_idx))], 'color', c_list{iF});
                        if ~isnan(msn_summary.hfr_ppc_diff(sidx)) % If the HFR-LFR frequency diff was significant
                                if ismember(msn_summary.hfr_ppc_diff(sidx), od.msn_res.near_spec{iC}.freqs(f_idx))
                                   scatter(ax{iF}, hi_mfr, mean(hfr_ppc(f_idx)), msz, c_list{iF})
                                end
                        end
                        if ~isnan(msn_summary.lfr_ppc_diff(sidx)) % If the HFR-LFR frequency diff was significant
                                if ismember(msn_summary.lfr_ppc_diff(sidx), od.msn_res.near_spec{iC}.freqs(f_idx))
                                    scatter(ax{iF}, low_mfr, mean(lfr_ppc(f_idx)), msz, c_list{iF})
                                end
                        end
                    end 
