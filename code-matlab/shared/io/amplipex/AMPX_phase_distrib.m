...
for itrial = trials_def;
    for ref = ref_chan;
        ...
        for ii = 1:64
            [Cxy,F] = cpsd(trial_data.trial{itrial}(ref,:),trial_data.trial{itrial}(ExpKeys.Probe_layout(ii),:),hamming(spec_window/2),spec_window/4,1024,trial_data.hdr.Fs);
            coh_spec_fake = -angle(Cxy); %higher value means leading.
            all_coh_spec{ref}(ii,1) = nanmean(coh_spec_fake(nearest(F,min_freq):nearest(F,max_freq)),1);
...
for ref = ref_chan
    ...
    for ii = 64:-1:1
        ...
        phase.phase_distrib_avg{ref} = reshape(phase_distrib_avg{ref}, 8,8);
    end
end
