...
if ~isstruct(ts_in)
    pass_flag = 0;
    fprintf('FAIL%s: input must be a ts datatype.\n',in_mfun);
else
    % check for .t field
    if ~isfield(ts_in,'t')
        pass_flag = 0;
        fprintf('FAIL%s: input ts must contain t field.\n',in_mfun);
        ...
        for iT = 1:length(ts_in.t)
            if ~iscolumn(ts_in.t{iT})
                pass_flag = 0;
                fprintf('FAIL%s: all contents of ts_in.t must be column vectors.\n',in_mfun);
            end
        end
    end
    ...
    if ~isfield(ts_in,'label')
        %pass_flag = 0;
        fprintf('WARNING%s: ts_in lacks a .label field.\n',in_mfun);
    else
        % check that label is the same length as t
        if length(ts_in.label) ~= length(ts_in.t)
            pass_flag = 0;
            fprintf('FAIL%s: ts_in.t and ts_in.label must have the same length.\n',in_mfun);
        end
    end
    ...
    if pass_flag && ~isfield(ts_in,'type')
        %pass_flag = 0;
        fprintf('WARNING%s: input appears to be ts datatype but lacks the .type field.\n',in_mfun);
    elseif pass_flag && isfield(ts_in,'type') && ~strcmp(ts_in.type,'ts')
        %pass_flag = 0;
        fprintf('WARNING%s: input appears to be ts datatype but is identified as %s in the .type field.\n',in_mfun,ts_in.type);
    end
