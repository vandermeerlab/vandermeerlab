function in = restrict3(in,varargin)
...
elseif isfield(in,'t') % ts
    type = 'ts';
    keep = cell(1,length(in.t)); %preallocation for speed
    
    for iC = 1:length(in.t)
        keep{iC} = false(size(in.t{iC})); %more preallocation for speed
        
        for iT = 1:length(iv_use.tstart)
            keep{iC}(in.t{iC} >= iv_use.tstart(iT) & in.t{iC} <= iv_use.tend(iT)) = 1;
        end
    end
